...
v_l = sign_v * v_l;
	...
	for (int i=1; i < 15000; i++)	{
		D = (double) i * h;
		lf = h * pdf_dist(entry_dist, D)         * pdf_prmot( sqrt(pow(v_l + delta_vl(entry_dist, D), 2) + pow(v_b, 2))        / (D )      /dang2vel , mu_c, mu_s ); 
		lc = h * pdf_dist(entry_dist, D + 0.5*h) * pdf_prmot( sqrt(pow(v_l + delta_vl(entry_dist, D + 0.5*h), 2) + pow (v_b,2))/((D+0.5*h))/dang2vel , mu_c, mu_s );
		lr = h * pdf_dist(entry_dist, D + 1.0*h) * pdf_prmot( sqrt(pow(v_l + delta_vl(entry_dist, D + 1.0*h), 2) + pow (v_b,2))/((D+1.0*h))/dang2vel , mu_c, mu_s );
		sum += (lf + 4 * lc + lr) / 6.;
	}
